...
function renderBoardShowcase(p, colors){
  ...
  const appHtml=apps.length?apps.map(a=>`<article class="bb-app-card">
      <div class="bb-app-head">
        <b>${esc(a.name||"Aplicação")}</b>
        <span>${esc(a.w)}×${esc(a.h)} ${esc(a.unit||"px")}</span>
      </div>
      <div class="bb-app-thumb">${a.svg?`<img src="${svgToDataUri(a.svg)}" alt="Preview ${esc(a.name||"Aplicação")}">`:`<div class="bb-app-empty">Sem arte</div>`}</div>
      <button class="bb-link" onclick="openAppEditor('${a.id}')">Abrir editor</button>
    </article>`).join(""):`<div class="empty-state" style="padding:16px"><p>Sem aplicações geradas.</p><button class="btn btn-ghost" onclick="createApplication()">Criar primeira aplicação</button></div>`;
...
function buildBrandBoardExportCSS(){
  ...
  .bb-app-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px}
  ...
  .bb-app-thumb{border:1px dashed var(--line);border-radius:10px;min-height:110px;background:var(--surface2);display:flex;align-items:center;justify-content:center;overflow:hidden}
  .bb-app-thumb img{width:100%;height:110px;object-fit:contain;display:block}
  .bb-app-empty{font-size:12px;color:#77809a}
  ...
  @media (max-width:1180px){.bb-showcase{grid-template-columns:1fr 1fr}.bb-card-wide{grid-column:span 2;}}
